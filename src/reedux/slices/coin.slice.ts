...
import { fetchCoinDetail, fetchCoins } from 'api';
...
export const fetchCoinsAsync = createAsyncThunk(
  ...
  async () => {
    ...
    return uniqBy(response.data, 'id');
  }
);

export const fetchCoinDetailAsync = createAsyncThunk(
  'coin/fetchDetail',
  async (id: string) => {
    const response = await fetchCoinDetail(id);
    // The value we return becomes the `fulfilled` action payload
...
export const coinSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCoinsAsync.rejected, (state) => {
        state.loading = false
      })
      .addCase(fetchCoinDetailAsync.pending, (state) => {
        state.loading = true;
      })
      .addCase(fetchCoinDetailAsync.fulfilled, (state, action) => {
        state.loading = false;
        state.coinDetail = action.payload;
      })
      .addCase(fetchCoinDetailAsync.rejected, (state) => {
        state.loading = false
      });
  },
});
...
export const selectCoinDetail = (state: RootState) => state.coin.coinDetail;
